classdef TracerDirector3 < mlraichle.TracerDirector2
	%% TRACERDIRECTOR3 
    ...
    methods (Static)
        function objs = migrateResolvedToVall(varargin)
            ...
            import mlraichle.TracerDirector3.migrationTeardown
            import mlraichle.TracerDirector3.CHART_TAG
            import mlsystem.DirTool
            
            this = TracerDirector3(mlpet.TracerResolveBuilder(varargin{:}));  
            ...
            for t = 1:length(tracers)
                for g = 1:length(tags)
                    dt = DirTool(sprintf('%sr2_op_%se1to*r1_frame*%s.4dfp.hdr', tracers{t}, tracers{t}, tags{g}));
                    if (dt.length > 0)
	...
	methods 
 		...
 		function this = TracerDirector3(varargin)
 			%% TRACERDIRECTOR3
 			%  @param .

 			this = this@mlraichle.TracerDirector2(varargin{:});
 		end
 	end 
