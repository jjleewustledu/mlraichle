classdef AugmentedAerobicGlycolysisKit < handle & mlpet.AbstractAerobicGlycolysisKit
	...
	methods (Static)        
        function construct(varargin)
            ...
            %  @param required physiology is char, e.g., cbv, cbf, cmro2, cmrglc.
    ...
    methods
        ...
        function [fs_,aifs_] = buildFsByWmparc1(this, varargin)
            ...
            for idx = this.indices % parcs
                ...
                fprintf('%s\n', datestr(now))
                fprintf('starting mlraichle.AugmentedAerobicGlycolysisKit.buildFsByWmparc1.idx -> %i\n', idx)
                roi = copy(wmparc1);
        ...
        function [ks_,cbv_,aifs_] = buildKsByWmparc1(this, varargin)
            ...
            for idx = this.indices % parcs
                ...
                % for parcs, build roibin as logical, roi as single 
                fprintf('%s\n', datestr(now))
                fprintf('starting mlraichle.AugmentedAerobicGlycolysisKit.buildKsByWmparc1.idx -> %i\n', idx)
                roi = copy(wmparc1);
                roi.fileprefix = sprintf('%s_index%i', roi.fileprefix, idx);
                roi = roi.numeq(idx);
                roi2 = copy(wmparc12);
        ...
        function [os_,aifs_] = buildOsByWmparc1(this, varargin)
            ...
            for idx = this.indices % parcs
                ...
                % for parcs, build roibin as logical, roi as single 
                fprintf('%s\n', datestr(now))
                fprintf('starting mlraichle.AugmentedAerobicGlycolysisKit.buildOsByWmparc1.idx -> %i\n', idx)
                roi = copy(wmparc1);
                roi.fileprefix = sprintf('%s_index%i', roi.fileprefix, idx);
                roi = roi.numeq(idx);
                roi2 = copy(wmparc12);
        ...
        function [vs_,aifs_] = buildVsByWmparc1(this, varargin)
            ...
            for idx = this.indices % parcs
                ...
                % for parcs, build roibin as logical, roi as single 
                fprintf('%s\n', datestr(now))
                fprintf('starting mlraichle.AugmentedAerobicGlycolysisKit.buildVsByWmparc1.idx -> %i\n', idx)
                roi = copy(wmparc1);
                roi.fileprefix = sprintf('%s_index%i', roi.fileprefix, idx);
                roi = roi.numeq(idx);
                roi2 = copy(wmparc12);
